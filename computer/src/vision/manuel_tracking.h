...
void manuel_tracking::settracker(cv::Ptr<cv::Tracker> t){
    _tracker = t;
}
cv::Ptr<cv::Tracker> manuel_tracking::gettracker(){
    return _tracker;
}
void manuel_tracking::settrackingbox(cv::Mat frame, bool f){
    _trackingBox = cv::selectROI(frame, f);
}
cv::Rect manuel_tracking::gettrackingbox(){
    return _trackingBox;
}

manuel_tracking::manuel_tracking(cv::Ptr<cv::Tracker> tracker):_tracker(tracker)
{
}
