...
def processFile(file):
    ...
    # Make count into speed using 0.1s intervals
    df.insert(4, "Speed", df["Count"])
    df["Speed"] = round(df["Count"].diff()/0.1,3)
    # Rename count since it's now an angle
    df.rename(columns={"Count": "Speed"}, inplace=True, errors="raise")
